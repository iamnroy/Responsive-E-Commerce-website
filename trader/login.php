...
if (isset($_POST['tlogin'])) {
  ...
  if (empty($email)) {
    ...
    header("Location:login.php?error=Invalid Email Format!! && $user_data");
    exit();
  } elseif (empty($pass1)) {
    ...
    $stid = oci_parse($conn, " SELECT * FROM TRADER WHERE email='$email' AND password='$pass1' AND STATUS='ENABLE'");
...
<body style="background: #E2F7ED">
  ...
  <div style="background: #E2F7ED" class="content">
    <form method="POST" action="login.php" style="background: pink">
      ...
      <label><a href="traderpw-reset.php" style="text-decoration: none"> Forgot Password?</a></label>
